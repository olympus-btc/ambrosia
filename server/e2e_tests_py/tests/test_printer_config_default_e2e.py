...
class TestPrinterConfigDefault:
    ...
    async def printer_config(self, admin_client):
        ...
        response = await admin_client.post(
            "/printers/configs",
            json={"printerType": PRINTER_TYPE, "printerName": f"e2e_printer_{uid}", "isDefault": False, "enabled": True},
        )
    ...
    async def test_set_default_succeeds(self, admin_client, printer_config):
        ...
        response = await admin_client.post(f"/printers/configs/{printer_config}/default")
    ...
    async def test_set_default_nonexistent_config_returns_404(self, admin_client):
        ...
        response = await admin_client.post(f"/printers/configs/{NONEXISTENT_ID}/default")
    ...
    @pytest.mark.asyncio
    async def test_set_default_marks_config_as_default(self, admin_client, printer_config):
    ...
    @pytest.mark.asyncio
    async def test_set_default_clears_previous_default(self, admin_client, printer_config):
        """Setting a new default clears the isDefault flag on the previous default config."""
        uid = str(uuid.uuid4())[:8]
        second = await admin_client.post(
            "/printers/configs",
            json={"printerType": PRINTER_TYPE, "printerName": f"e2e_second_{uid}", "isDefault": False, "enabled": True},
        )
        ...
        try:
            ...
            assert second_cfg["isDefault"] is True, "Second config should now be default"
            assert first["isDefault"] is False, "First config should no longer be default"
