...
class TestDishNameValidation:
    ...
    @pytest.mark.asyncio
    async def test_create_dish_with_blank_name_returns_201_with_null_id(
        ...
        assert_status_code(
            response, 201, "Blank dish name on create currently returns 201 (known bug)"
        )
        assert response.json()["id"] is None, "Expected null id for blank dish name"
        logger.info(
            "✓ Blank dish name on create returns 201 with null id (current behaviour)"
        )

    @pytest.mark.asyncio
    async def test_create_dish_with_whitespace_name_returns_201_with_null_id(
        self, admin_client, category_id
    ):
        ...
        assert_status_code(
            response,
            201,
            "Whitespace-only dish name on create currently returns 201 (known bug)",
        )
        assert response.json()["id"] is None, (
            "Expected null id for whitespace-only dish name"
        )
        logger.info(
            "✓ Whitespace-only dish name on create returns 201 with null id (current behaviour)"
        )

    @pytest.mark.asyncio
    async def test_create_dish_with_valid_name_succeeds(
        self, admin_client, category_id
    ):
        ...
        response = await admin_client.post(
            "/dishes",
            json={
                "name": f"valid_dish_{uid}",
                "price": 10.0,
                "category_id": category_id,
            },
        )
        assert_status_code(
            response, 201, "Valid dish name should be accepted on create"
        )
        await admin_client.delete(f"/dishes/{response.json()['id']}")
        logger.info("✓ Valid dish name correctly accepted on create")

    @pytest.mark.asyncio
    async def test_update_dish_with_blank_name_returns_404(
        self, admin_client, existing_dish
    ):
        ...
        assert_status_code(
            response, 404, "Blank dish name on update currently returns 404"
        )
        logger.info("✓ Blank dish name on update returns 404 (current behaviour)")

    @pytest.mark.asyncio
    async def test_update_dish_with_whitespace_name_returns_404(
        self, admin_client, existing_dish
    ):
        ...
        assert_status_code(
            response, 404, "Whitespace-only dish name on update currently returns 404"
        )
        logger.info(
            "✓ Whitespace-only dish name on update returns 404 (current behaviour)"
        )

    @pytest.mark.asyncio
    async def test_update_dish_with_valid_name_succeeds(
        self, admin_client, existing_dish, category_id
    ):
        ...
        response = await admin_client.put(
            f"/dishes/{existing_dish}",
            json={
                "name": f"updated_dish_{uid}",
                "price": 10.0,
                "category_id": category_id,
            },
        )
        assert_status_code(
            response, 200, "Valid dish name should be accepted on update"
        )
