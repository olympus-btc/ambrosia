...
class TestRoleNameValidation:
    ...
    async def test_create_role_with_blank_name_fails(self, admin_client):
        ...
        assert_status_code(response, 400, "Blank role name should be rejected on create")
    ...
    async def test_create_role_with_whitespace_name_fails(self, admin_client):
        ...
        response = await admin_client.post("/roles", json={"role": "   "})
        assert_status_code(response, 400, "Whitespace-only role name should be rejected on create")
    ...
    async def test_create_role_with_valid_name_succeeds(self, admin_client):
        ...
        assert_status_code(response, 201, "Valid role name should be accepted on create")
    ...
    async def test_update_role_with_blank_name_fails(self, admin_client, existing_role):
        ...
        assert_status_code(response, 400, "Blank role name should be rejected on update")
        logger.info("✓ Blank role name correctly rejected on update")

    @pytest.mark.asyncio
    async def test_update_role_with_whitespace_name_fails(self, admin_client, existing_role):
        """PUT /roles/{id} with a whitespace-only name should return 400."""
        response = await admin_client.put(f"/roles/{existing_role}", json={"role": "   "})
        assert_status_code(response, 400, "Whitespace-only role name should be rejected on update")
        logger.info("✓ Whitespace-only role name correctly rejected on update")

    @pytest.mark.asyncio
    async def test_update_role_with_valid_name_succeeds(self, admin_client, existing_role):
        """PUT /roles/{id} with a valid name should return 200."""
        uid = str(uuid.uuid4())[:8]
        response = await admin_client.put(f"/roles/{existing_role}", json={"role": f"updated_role_{uid}"})
        assert_status_code(response, 200, "Valid role name should be accepted on update")
