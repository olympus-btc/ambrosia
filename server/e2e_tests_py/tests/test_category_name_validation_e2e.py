...
class TestCategoryNameValidation:
    ...
    async def test_create_category_with_blank_name_fails(self, admin_client):
        ...
        assert_status_code(
            response, 400, "Blank category name should be rejected on create"
        )
    ...
    async def test_create_category_with_whitespace_name_fails(self, admin_client):
        ...
        assert_status_code(
            response, 400, "Whitespace-only category name should be rejected on create"
        )
    ...
    async def test_create_category_with_valid_name_succeeds(self, admin_client):
        ...
        assert_status_code(
            response, 201, "Valid category name should be accepted on create"
        )
        await admin_client.delete(
            f"/categories/{response.json()['id']}?type={VALID_TYPE}"
        )
        logger.info("✓ Valid category name correctly accepted on create")

    @pytest.mark.asyncio
    async def test_update_category_with_blank_name_returns_404(
        self, admin_client, existing_category
    ):
        ...
        assert_status_code(
            response, 404, "Blank category name on update currently returns 404"
        )
        logger.info("✓ Blank category name on update returns 404 (current behaviour)")

    @pytest.mark.asyncio
    async def test_update_category_with_whitespace_name_returns_404(
        self, admin_client, existing_category
    ):
        ...
        assert_status_code(
            response,
            404,
            "Whitespace-only category name on update currently returns 404",
        )
        logger.info(
            "✓ Whitespace-only category name on update returns 404 (current behaviour)"
        )

    @pytest.mark.asyncio
    async def test_update_category_with_valid_name_succeeds(
        self, admin_client, existing_category
    ):
        ...
        assert_status_code(
            response, 200, "Valid category name should be accepted on update"
        )
