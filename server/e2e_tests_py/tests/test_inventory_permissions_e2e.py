...
class TestDishesPermissions:
    """Permission enforcement tests for /dishes."""

    @pytest.mark.asyncio
    async def test_dishes_read_required_for_get(self, client_factory):
        """GET /dishes returns 403 without dishes_read permission."""
        no_perm = await client_factory(permissions=[])
        assert_status_code(await no_perm.get("/dishes"), 403)

        with_perm = await client_factory(permissions=["dishes_read"])
        assert (await with_perm.get("/dishes")).status_code != 403
        logger.info("✓ dishes_read correctly gates GET /dishes")
    ...
    @pytest.mark.asyncio
    async def test_dishes_delete_required_for_delete(self, client_factory):
        """DELETE /dishes/{id} returns 403 without dishes_delete permission."""
        no_perm = await client_factory(permissions=[])
        assert_status_code(await no_perm.delete(f"/dishes/{DUMMY_ID}"), 403)

        with_perm = await client_factory(permissions=["dishes_delete"])
        assert (await with_perm.delete(f"/dishes/{DUMMY_ID}")).status_code != 403
        logger.info("✓ dishes_delete correctly gates DELETE /dishes/{id}")
