...
describe("useReports — computed analytics", () => {
  const salesReport = {
    totalRevenueCents: 7000,
    totalItemsSold: 5,
    sales: [
  ...
  async function setupWithSales() {
    ...
    jest.clearAllMocks();
    httpClient.mockResolvedValue({});
    parseJsonResponse.mockResolvedValueOnce(salesReport);
    await act(async () => { await hook.result.current.fetchReport({ period: "month" }); });
    return hook;
  }

  it("sales is [] when reportData is null", () => {
    parseJsonResponse.mockResolvedValue({ totalRevenueCents: 0, totalItemsSold: 0, sales: [] });
    const { result } = renderHook(() => useReports());
    expect(result.current.sales).toEqual([]);
  });

  it("sales derives from reportData.sales", async () => {
    const { result } = await setupWithSales();
    expect(result.current.sales).toEqual(salesReport.sales);
  });

  it("revenueByDay groups sales by date and sums quantity * priceAtOrder", async () => {
    const { result } = await setupWithSales();
    const jan2 = result.current.revenueByDay.find((d) => d.date === "2024-01-02");
    expect(jan2.revenue).toBe(3000); // 2*1000 + 1*1000
  });

  it("revenueByDay is sorted ascending by date", async () => {
    const { result } = await setupWithSales();
    const dates = result.current.revenueByDay.map((d) => d.date);
    expect(dates).toEqual([...dates].sort());
  });

  it("revenueByDay count tracks total quantity per day", async () => {
    const { result } = await setupWithSales();
    const jan2 = result.current.revenueByDay.find((d) => d.date === "2024-01-02");
    expect(jan2.count).toBe(3); // quantity 2 + 1
  });

  it("topProducts sorts by revenue descending", async () => {
    const { result } = await setupWithSales();
    const revenues = result.current.topProducts.map((p) => p.revenue);
    expect(revenues).toEqual([...revenues].sort((a, b) => b - a));
  });

  it("topProducts aggregates quantity across multiple sales of the same product", async () => {
    const { result } = await setupWithSales();
    const productA = result.current.topProducts.find((p) => p.name === "A");
    expect(productA.quantity).toBe(3);
  });

  it("topProducts is capped at 8 entries", async () => {
    ...
    jest.clearAllMocks();
    httpClient.mockResolvedValue({});
    parseJsonResponse.mockResolvedValueOnce({ totalRevenueCents: 0, totalItemsSold: 0, sales: manySales });
    await act(async () => { await hook.result.current.fetchReport({ period: "month" }); });
    expect(hook.result.current.topProducts).toHaveLength(8);
  });

  it("paymentMethodSplit groups by paymentMethod and sorts by revenue descending", async () => {
    const { result } = await setupWithSales();
    const [first] = result.current.paymentMethodSplit;
    expect(first.method).toBe("Cash"); // Cash: 3000, BTC: 3000 — Cash has higher count but same revenue; order is stable
  ...
  it("paymentMethodSplit count tracks number of sale records per method", async () => {
    const { result } = await setupWithSales();
